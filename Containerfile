...
# ⚠ THE ROCKSDB PAIR, PINNED BY DIGEST. The letter read model runs on
...
# build image, and mcl_mail_service_tests guards all three pins.
...
WORKDIR /app
COPY --from=builder /build/_build/prod/rel/mcl_mail ./
